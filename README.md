...
A sincronização de contas está habilitada (`syncEnabled: true`). Alterações são gravadas localmente e enviadas automaticamente; o indicador distingue gravação local, pendências offline, confirmação do servidor e conflitos. As preferências de treino acompanham a conta, enquanto a sessão ativa permanece específica do dispositivo. Dados de visitante ficam locais e só são adicionados à conta após prévia e confirmação explícitas. Contas com dados locais anteriores também podem exigir essa revisão inicial. Não há confirmação a cada resolução nem botão obrigatório para sincronizar. Mantenha backups antes de trocar dispositivo ou limpar dados do navegador; `localhost` e `127.0.0.1` têm armazenamentos distintos.

O formato ativo/exportado é v3. A leitura de v1/v2 preserva sessões, solves, preferências, progresso e estudo, acrescentando modalidade não classificada aos registros anteriores. Não se presume Duas mãos. Classificar exige confirmar que a sessão inteira pertence à modalidade; sessões misturadas ou incertas podem permanecer não classificadas. No primeiro uso de um espaço visitante comprovadamente novo, o repositório cria **Treino diário**, em **Duas mãos**, com acesso ao seletor Uma mão. Sessões existentes, modalidade não classificada e dados corrompidos não acionam esse padrão; excluir a última sessão não o recria. Outras novas sessões exigem modalidade explícita. Em contas, o padrão só é criado após carregar os dados remotos e confirmar que o espaço é novo. A fonte original `nexus-cube:v1` é preservada. O repositório usa IndexedDB e separa os dados de visitante dos espaços de cada identidade; entrar não adota nem envia os registros de visitante. Importações são validadas antes da gravação. Dados corrompidos e falhas de armazenamento são apresentados ao usuário.
...
node --import tsx --test \
  ...
  tests/domain/solver/validation.test.ts \
  tests/domain/solver/solution.test.ts \
  tests/domain/solver/client.test.ts \
  tests/domain/solver/worker-lifecycle.test.ts \
  tests/domain/solver/cfop.test.ts \
  tests/domain/solver/roux/plan.test.ts \
  tests/domain/solver/roux/stages.test.ts \
  tests/qa/solver-oracle-contract.test.ts \
  tests/qa/solver-validation-integration.test.ts \
  tests/qa/solver-real-oracle.test.ts \
  tests/qa/solver-stage-oracle.test.ts \
  tests/qa/solver-method-plan-contract.test.ts \
  ...
  tests/cloud/sync-action-inputs.test.ts \
  tests/security/sync-gate-boundary.test.ts \
  tests/security/sync-reconciliation-boundary.test.ts \
  tests/security/sync-input-response-boundary.test.ts \
  tests/ui/auth-callback.test.ts \
  tests/security/auth-callback-entry-boundary.test.ts \
  tests/qa/automatic-sync-v3-service-focus.test.ts \
  tests/qa/sync-a1-a2-propagation.test.ts
npm run build
```

Em um workspace compartilhado, combine a pausa dos escritores e a janela de integração antes da suíte completa ou build. O comando principal lista **75 arquivos, com 294 testes aprovados**, reunindo a base Wizard de 67 arquivos e os focos de sync, propagação entre duas cópias da conta e recovery. Os cenários ON e OFF são executados sem filtro; nenhum caso ON foi removido. O build terminou com exit 0 e gerou 53 arquivos. Execute cada comando seguinte somente se o anterior terminar com exit 0. SQL, V4, administração e o baseline arquivado em `tests/qa/fixtures-v1-baseline/` ficam fora deste comando. Não substitua a lista por um glob de todos os testes. Os testes usam fixtures sintéticas e oráculos independentes; não são dados iniciais do aplicativo. Consulte [os testes de QA](tests/qa/README.md).
...
Cadastro, login, confirmação, recuperação, logout e refresh estão implementados; a validação com contas reais e email ainda está pendente. Nesta fase, o cadastro usa email e senha. No projeto desta entrega, a confirmação de email desativada foi verificada em 8 de setembro de 2026 às 19:03:33 UTC. Uma sessão retornada pelo cadastro permite entrada imediata; uma eventual exigência de confirmação é tratada somente se o serviço a retornar. A configuração SMTP e seus parâmetros foram verificados às 19:06:01 UTC, mantendo a confirmação desativada. Isso não comprova entrega de email nem recuperação de senha: nenhum envio real foi validado nesta rodada. A URL de produção e os callbacks autorizados foram configurados e conferidos; o fluxo real por email ainda não foi validado. A interface não promete entrega.

O backend de sync v3 foi instalado e conferido: nove RPCs e oito tabelas com RLS obrigatória, sem administração ou V4 nesta fatia. A sincronização automática e a adoção explícita de visitante usam esse contrato. Gravação local não significa confirmação do servidor; conflitos exigem uma escolha explícita, e dados e filas são preservados em caso de falha. A inspeção do backend e os testes sintéticos não comprovam todos os fluxos entre dispositivos com contas reais, entrega de email ou a capacidade física máxima. O fluxo completo de importação Cube Timer/csTimer e a área administrativa continuam fora desta entrega. A recuperação técnica de backup de visitante fica no Timer somente quando seu armazenamento está corrompido ou bloqueado, com a fonte original preservada.
