...
export type SolverMethod='cfop'|'roux'|'lbl';
export type F2LSlot='FR'|'FL'|'BR'|'BL';
export type MethodStageId='cfop.cross'|`cfop.f2l.${F2LSlot}`|'cfop.oll'|'cfop.pll'|'cfop.auf'|'roux.fb'|'roux.sb'|'roux.cmll'|'roux.cmll-auf'|'roux.eo'|'roux.lr'|'roux.finish'|'lbl.cross'|'lbl.corners'|'lbl.middle'|'lbl.top-cross'|'lbl.top-edges'|'lbl.top-corners-position'|'lbl.top-corners-orient';
export type MethodGoal={kind:'cross'|'f2l'|'oll'|'pll-up-to-auf'|'first-block'|'second-block'|'cmll'|'cmll-up-to-auf'|'lse-eo'|'lse-lr'|'solved'|'first-layer'|'ll-edges-oriented'|'ll-edges-solved'|'ll-corners-placed'}|{kind:'f2l-pair';slot:F2LSlot};
