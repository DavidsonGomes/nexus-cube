...
export async function planRoux(input: ValidatedSolverInput, options: MethodPlannerOptions = {}): Promise<MethodPlan> {
  ...
  const fb = simplifySolverAlgorithm(await searchBlock(builder.state, ['DL', 'FL', 'BL', 'DFL', 'DBL'], turns('URFDLB'), context));
  ...
  const sb = simplifySolverAlgorithm(await searchBlock(builder.state, ['DR', 'FR', 'BR', 'DFR', 'DBR', 'U'], turns('URMr'), context));
  ...
  for (const part of ['eo', 'lr', 'finish'] as const) {
    await checkpoint(context);
    const algorithm = simplifySolverAlgorithm(await searchLSE(builder.state, part, context));
    const details = {
