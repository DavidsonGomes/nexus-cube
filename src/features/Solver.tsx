...
type Job = {requestId:string; inputKey:string; scheme:string; method:SolverMode; revision:number; context:ContextHandle; controller:AbortController};
...
export default function SolverPage({createClient = createSolverClient, availableMethods = releasedMethods}: {createClient?:()=>SolverClient; availableMethods?:readonly SolverMode[]}) {
  ...
  function current(candidate:Job) {
    return compatibleScheme && candidate.scheme === SOLVER_FRAME && candidate.inputKey.startsWith(SOLVER_FRAME + ':') && mounted.current && job.current === candidate && revision.current === candidate.revision && authorized() && sameContext(candidate.context, service.getSnapshot().context);
  }
  function stop() {
  ...
  async function solve() {
    ...
    const candidate:Job = {requestId:crypto.randomUUID(), inputKey:validation.inputKey, scheme:SOLVER_FRAME, method, revision:revision.current, context:{...context}, controller:new AbortController()};
  ...
  async function copySolution() {
    ...
    if(!compatibleScheme || !captured || !captured.inputKey.startsWith(SOLVER_FRAME + ':') || !context || !authorized() || !sameContext(context, service.getSnapshot().context))return;
