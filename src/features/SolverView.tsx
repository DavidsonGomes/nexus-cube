...
import {createSolverClient, createEmptyDraft, getDraftPreview, SOLVER_FACES, SOLVER_FRAME, validateDraft} from '../solver';
...
type Job = {requestId:string; inputKey:string; scheme:string; method:SolverMode; revision:number; context:ContextHandle; authorityOwner:object; controller:AbortController};
...
export default function SolverView(props:SolverViewProps) {
  ...
  const boundary=useRef({owner:authority.owner,context:authority.context?{...authority.context}:null,authorized:authority.authorized,epoch:0});
  const previous=boundary.current;
  if(previous.owner!==authority.owner||previous.authorized!==authority.authorized||!(previous.context===null&&authority.context===null||previous.context&&sameContext(previous.context,authority.context))) {
    boundary.current={owner:authority.owner,context:authority.context?{...authority.context}:null,authorized:authority.authorized,epoch:previous.epoch+1};
  }
  // Scope temporary colors/results to the authority, including same-user
  // generation and provider replacement, before any child is rendered.
  if(!authority.authorized||!authority.context?.userId)return null;
  return <SolverWorkspace key={boundary.current.epoch} {...props}/>;
}
function SolverWorkspace({createClient=createSolverClient,availableMethods=releasedMethods,authority,speedControl}:SolverViewProps) {
  const authorityRef=useRef(authority);authorityRef.current=authority;
  const notifyForContext=(text:string,context:ContextHandle)=>{const active=authorityRef.current;if(active.owner===authority.owner&&active.current().authorized&&sameContext(context,active.current().context))active.notifyForContext(text,context);};
  const [draftState, setDraft] = useState<DraftEnvelope | DraftFacelets>(() => ({scheme:SOLVER_FRAME, draft:createEmptyDraft()}));
  const draft = 'scheme' in draftState ? draftState.draft : draftState;
  const compatibleScheme = 'scheme' in draftState && draftState.scheme === SOLVER_FRAME;
  const [storedSolution, setSolution] = useState<Solution|null>(null);
  const solution = compatibleScheme && storedSolution?.inputKey.startsWith(SOLVER_FRAME + ':') ? storedSolution : null;
  ...
  const validation = useMemo(() => compatibleScheme ? validateDraft(draft) : null, [draft, compatibleScheme]);
  const preview = useMemo(() => compatibleScheme ? getDraftPreview(draft) : null, [draft, compatibleScheme]);
  ...
  const issues = !validation || validation.kind === 'valid' ? [] : validation.issues;
  function authorized() {return authorityRef.current.current().authorized;}
  function current(candidate:Job) {
    return compatibleScheme && candidate.scheme === SOLVER_FRAME && candidate.inputKey.startsWith(SOLVER_FRAME + ':') && mounted.current && job.current === candidate && revision.current === candidate.revision && candidate.authorityOwner === authorityRef.current.owner && authorized() && sameContext(candidate.context, authorityRef.current.current().context);
  }
  function stop() {
  ...
  function chooseMethod(next:SolverMode) {
    if (!compatibleScheme || next === method || !availableMethods.includes(next)) return;
  ...
  function replaceDraft(next:DraftFacelets) {
    stop(); revision.current++; setSolution(null); setError(''); setStatus(solution || phase ? 'As cores mudaram. Calcule outra solução para este estado.' : ''); setDraft({scheme:SOLVER_FRAME, draft:next}); setResetOpen(false);
  }
  function newDraft() {setReviewReady(false);setWizardEpoch(value=>value+1);replaceDraft(createEmptyDraft());}
  function paint(face:Face, index:number, color:Face|null) {
    if (!compatibleScheme || index === 4 || draft[face][index] === color) return;
  ...
  useEffect(() => {if (!compatibleScheme) {job.current?.controller.abort(); job.current = null; client.current?.cancel();}}, [compatibleScheme]);
  async function solve() {
    const context = authorityRef.current.current().context;
    if (!reviewReady || !compatibleScheme || !context || !context.userId || !authorized() || validation?.kind !== 'valid' || job.current || !availableMethods.includes(method)) return;
    const candidate:Job = {requestId:crypto.randomUUID(), inputKey:validation.inputKey, scheme:SOLVER_FRAME, method, revision:revision.current, context:{...context}, authorityOwner:authorityRef.current.owner, controller:new AbortController()};
  ...
  async function copySolution() {
    const context=authority.context, captured=solution;
    if(!compatibleScheme || !captured || !captured.inputKey.startsWith(SOLVER_FRAME + ':') || !context || !authorized() || !sameContext(context, authorityRef.current.current().context))return;
  ...
  if(!authority.context?.userId || !authority.authorized) return null;
  if(!compatibleScheme || !validation || !preview) return <section className="panel solver-scheme-transition" role="alert" data-testid="solver-scheme-transition"><h2>O esquema de cores foi corrigido</h2><p>Com amarelo em cima e verde à frente, vermelho fica à esquerda e laranja à direita. Este preenchimento pertence a uma referência anterior ou sem versão e não foi convertido.</p><p>As {filled} posições preenchidas continuam somente na memória desta página. Não calcularemos uma solução nem mostraremos suas cores sob a nova referência. A versão anterior não salvava o preenchimento; sair ou recarregar pode perdê-lo.</p>{resetOpen ? <div><p>Iniciar novamente apagará este preenchimento temporário. Confira seu cubo físico antes de começar.</p><button className="button secondary" onClick={() => setResetOpen(false)}>Manter preenchimento anterior</button><button className="button" onClick={newDraft}>Confirmar novo preenchimento</button></div> : <button className="button" onClick={() => setResetOpen(true)}>Iniciar novo preenchimento</button>}</section>;
  return <div className="solver-page" data-testid="solver-page">
    <section className="solver-intro"><span className="solver-intro-icon"><ShieldCheck size={23}/></span><div><strong>Comece por aqui</strong><p>Segure o cubo com o centro <b>amarelo em cima</b>, <b>verde à frente</b> e <b>vermelho à esquerda</b>. Os 6 centros já estão definidos. Preencha as outras 48 posições para representar os 54 adesivos. Confira o passo inicial e execute a sequência no seu cubo. O cálculo acontece neste dispositivo.</p></div></section>
    {reviewReady && <section className="solver-mode-section" aria-label="Escolha do método"><div className="solver-mode-picker">{modes.map(mode => <button key={mode.id} className={method === mode.id ? 'selected' : ''} aria-pressed={method === mode.id} disabled={!availableMethods.includes(mode.id)} onClick={() => chooseMethod(mode.id)}><strong>{mode.title}</strong><span>{mode.description}</span>{!availableMethods.includes(mode.id) && <small>Em implementação e validação</small>}</button>)}</div><p className="small muted">Nenhum modo promete a menor sequência. Trocar o método reinicia a reprodução a partir das cores informadas.</p></section>}
    <div className={`solver-workspace ${reviewReady ? '' : 'solver-workspace-guided'}`}>
      <section className="panel solver-editor-panel"><SolverWizard key={wizardEpoch + ':' + SOLVER_FRAME} draft={draft} onPaint={paint} issues={issues} onReviewChange={value=>{setReviewReady(value);if(!value)stop();}}/>
        ...
        {resetOpen && <div className="solver-reset-confirm"><p>Apagar as cores preenchidas? Os seis centros continuam fixos.</p><div><button className="button secondary" onClick={() => setResetOpen(false)}>Manter</button><button className="button" onClick={newDraft}>Limpar preenchimento</button></div></div>}
      </section>
      {reviewReady && <aside className="solver-preview-column">
