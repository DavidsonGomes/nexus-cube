...
import { invalid, jsonFailure, MAX_IMPORT_SOURCE_BYTES, parseJson, record } from './shared';
...
export function inspectImportBytes(bytes:Uint8Array):ImportParseResult {
  ...
  let root;
  try{root=parseJson(text);}catch(error){return jsonFailure(error,'invalid-json','Arquivo não contém JSON válido dentro dos limites.');}
