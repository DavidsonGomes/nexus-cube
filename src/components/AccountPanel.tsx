...
import type {AuthResult,LogoutResult} from '../cloud/types';

type Screen='signin'|'signup'|'reset';
export default function AccountPanel({onClose,requestedFeature}:{onClose:()=>void;requestedFeature?:string}){
 const {cloud,authAction,timerBusy,saving}=useData();
 ...
 async function perform(action:Parameters<typeof authAction<AuthResult|LogoutResult|{kind:'guest'}>>[0]){
   if(blocked)return;setError('');setMessage('');setPending(true);
   const result=await authAction(action);
   if(!mounted.current)return;
   setPending(false);setPassword('');setRepeat('');
   switch(result.kind){
    case 'error':setError(result.message);break;
    case 'authenticated':setMessage(screen==='signup'?'Conta criada. Você já está conectado. Seus treinos ficam neste dispositivo.':'Conta conectada. Seus dados desta conta ficam neste dispositivo.');break;
 ...
 function submit(event:FormEvent){event.preventDefault();if(blocked)return;
   ...
   else if(screen==='signup')void perform(service=>service.signUp({email:email.trim(),password}));
   else void perform(service=>service.signIn({email:email.trim(),password}));
 }
 ...
 return <div className="modal-backdrop account-backdrop"><section className="modal account-panel" role="dialog" aria-modal="true" aria-labelledby="account-title">
   ...
   {account?<>
    ...
    <div className="account-local"><CloudOff size={22}/><div><strong>Dados locais neste dispositivo</strong><p>Seus treinos desta conta ficam separados dos registros de visitante. A sincronização entre dispositivos ainda não está disponível. Exporte backups nas configurações.</p></div></div>
    ...
    <p className="account-privacy"><LockKeyhole size={15}/> Entrar não transfere seus dados de visitante. Nesta versão, os treinos de cada conta ficam apenas neste dispositivo.</p>
   </>}
