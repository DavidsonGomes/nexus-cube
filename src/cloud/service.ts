import { createInitialData, exportBackup, loadData, saveData, type StorageLike } from '../data';
...
export interface CloudServicePorts { projectRef: string; configured?: boolean; authFactory: AuthDriverFactory; store: CloudAtomicStore; guestStorage?: StorageLike; online?: () => boolean; redirectTo: string; now?: () => number; randomId?: () => string }
...
export function createCloudServiceWithPorts(ports: CloudServicePorts): CloudService {
  ...
  let snapshot: CloudSnapshot = { status: 'initializing', identity: null, context: null, data: null, localRevision: 0, error: null, recoveryContextId: null, sync: 'unavailable', accountImport: 'unavailable', storageLabel: 'Dados locais neste dispositivo' };
  ...
  function hide(error: Failure | null = null) { emit({ status: 'locked', identity: null, context: null, data: null, localRevision: 0, recoveryContextId: null, error }); }
  ...
  async function activate(instance: string, generation: number, user: CloudIdentity): Promise<AuthResult> {
    ...
    const state = await store.transact(current => {
      ...
      if (!hasOwn(current.accounts, user.id)) current.accounts[user.id] = { data: createInitialData(), revision: 0 };
    ...
    memoryLocked = false; verifiedUserId = user.id; publish(current); return { kind: 'authenticated', identity: user };
  }
  ...
  const service: CloudService = {
    ...
    initialize: () => initialization ??= (async () => {
      ...
      try {
        ...
        if (!state.guest) {
          const guest = loadData(ports.guestStorage);
          state = await store.transact(current => { if (!current.guest) { current.guest = { data: checked(guest.data), revision: 0 }; current.guestError = guest.error; } return structuredClone(current); });
        }
        if (state.gate === 'guest') publish(state);
          ...
          if (user?.id === state.user?.id) { const current = await store.read(); if (current.generation === state.generation) { verifiedUserId = user!.id; publish(current); } }
    ...
    commit: async input => {
      try { const result = await store.transact(state => { guard(state, input.context); assertIdle(state); if (state.gate === 'guest' && state.guestError) denied('invalid', state.guestError); const current = workspace(state); if (current.revision !== input.expectedLocalRevision) denied('stale-local', 'Os dados mudaram em outra aba. Revise e tente novamente.'); const next = checked(typeof input.change === 'function' ? input.change(structuredClone(current.data)) : input.change); current.data = next; current.revision++; return { kind: 'committed' as const, data: next, localRevision: current.revision, context: handle(state), sync: 'unavailable' as const }; }); return await returnForContext(input.context, result, true); } catch (error) { return failure(error); }
    },
    ...
    finishCapture: async input => { try { const result: CommitResult = await store.transact(state => { guard(state, input.capture.context); const draft = eligibleDraft(state, input.capture); const current = workspace(state); if (draft.state === 'committed') return { kind: 'committed', data: current.data, localRevision: current.revision, context: handle(state), sync: 'unavailable' }; if (draft.state !== 'completed' || !draft.solve || !draft.source) denied('invalid', 'Resultado durável com metadados completos não disponível.'); if (current.revision !== input.expectedLocalRevision) denied('stale-local', 'Os dados mudaram. Revise a captura antes de tentar novamente.'); if (current.data.solves.some(solve => solve.id === draft.solve!.id)) denied('invalid', 'O identificador da captura já existe. O draft foi preservado.'); const next = checked({ ...current.data, solves: [...current.data.solves, draft.solve] }); current.data = next; current.revision++; draft.state = 'committed'; return { kind: 'committed', data: next, localRevision: current.revision, context: handle(state), sync: 'unavailable' }; }); return await returnForContext(input.capture.context, result, true); } catch (error) { return failure(error); } },
    ...
    dispose: () => { disposed = true; unsubscribeStore(); disposeDrivers(); listeners.clear(); store.close(); },
  };
  return service;
}
