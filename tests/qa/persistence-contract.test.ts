...
test('A4: backup restores complete independent fixture in one write', () => {
  const data = qaData();
  const payloadOriginal = structuredClone(data);
  const serialized = exportBackup(data), storage = new MemoryStorage('previous QA value');
  const expected = { ...payloadOriginal, version: 3 as const };
  assert.deepEqual(parseBackup(serialized), expected);
  assert.deepEqual(importBackup(serialized, storage), expected);
  assert.deepEqual(data, payloadOriginal, 'backup operations must not mutate the v3 payload');
  assert.equal(storage.writes.length, 1);
  assert.equal(storage.writes[0][0], QA_KEY);
  assert.deepEqual(loadData(storage), { data: expected, error: null });
});
...
test('A4: CSV contains escaped quotes, newline, Unicode and session scope', () => {
  const data = qaData();
  const csv = exportCSV(data, {kind:'all'});
  assert.ok(csv.includes('""aspas""')); assert.ok(csv.includes('segunda linha 🧊'));
  assert.ok(csv.includes('DNF')); assert.ok(csv.includes('+2'));
  const onlyA = exportCSV(data, {kind:'session', sessionId:'qa-a', mode:null}); assert.ok(!onlyA.includes('segunda linha 🧊'));
});
